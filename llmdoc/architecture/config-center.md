...
| `setting/model_setting/global.go` | `GlobalSettings.OpenAIDownstreamStrictUpstreamEnabled` | OpenAI 下游严格上游限制开关（`global.*` 分层配置的一部分） |
...
| `web/src/pages/Setting/Model/SettingGlobalModel.jsx` | `SettingGlobalModel` | 管理端保存 `global.openai_downstream_strict_upstream_enabled` 等全局模型配置 |
...
6. 对 `global.*` 这类分层配置（例如 `global.openai_downstream_strict_upstream_enabled`），`updateOptionMap` 会走 `ConfigManager` 分发到已注册模块，并热更新到运行态结构体。`model/option.go:482-522`, `setting/model_setting/global.go:33-55`  
...
- `global.*` 分层配置的 value 仍以字符串持久化；布尔型开关应使用 `true/false` 文本，JSON 结构建议保存格式化后的字符串以便排障。`web/src/pages/Setting/Model/SettingGlobalModel.jsx:148-175`, `model/option.go:482-522`  
