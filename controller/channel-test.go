...
func testChannel(channel *model.Channel, testModel string, endpointType string, isStream bool, forceBaseURLID int) testResult {
	...
	if !service.IsAPITypeAllowedForRelayFormat(apiType, relayFormat) {
		return testResult{
			context:     c,
			localErr:    fmt.Errorf("relay format %s only supports openai/codex compatible upstream, got api type %d", string(relayFormat), apiType),
...
func testAllChannels(notify bool) error {
	...
	gopool.Go(func() {
		...
		for _, channel := range channels {
			...
			tik := time.Now()
			result := testChannel(channel, "", "", shouldUseStreamForAutomaticChannelTest(channel), 0)
			tok := time.Now()
			milliseconds := tok.Sub(tik).Milliseconds()
