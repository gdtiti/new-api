...
func applyResponsesWSPassthroughHeaders(c *gin.Context, info *common.RelayInfo, targetHeader http.Header) {
	if c == nil || c.Request == nil || info == nil || targetHeader == nil {
		return
	}
	if info.RelayMode != constant.RelayModeResponses {
		return
	}
	rules := info.ChannelOtherSettings.OpenAIResponsesWSPassthroughHeaders
	if len(rules) == 0 {
		return
	}
	...
	for _, rule := range rules {
		...
		if name == "" || shouldSkipPassthroughHeader(name) {
			continue
		}
		lower := strings.ToLower(name)
...
func DoWssRequest(a Adaptor, c *gin.Context, info *common.RelayInfo, requestBody io.Reader) (*websocket.Conn, error) {
	...
	applyResponsesWSPassthroughHeaders(c, info, targetHeader)
